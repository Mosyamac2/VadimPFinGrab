...
JS-challenge). Для регулярных прогонов с VPS либо подсуньте свежие
cookies в `app.yaml → discoverer.cookies` (живут несколько часов), либо
поднимите headless-Chromium как отдельный proxy (за пределами v2 patch
series).
