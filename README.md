...
> **Текущий статус (на 2026-05-02).** v1 (этапы 01–15), v2 (Patch 16–22, адаптация
> под боевой e-disclosure.ru) и v3 (Patch 23–28, production-hardening после
> live-прогонов на VPS) реализованы и помержены. Discoverer ходит через
> headless-Chromium (Playwright) для прохождения ServicePipe JA3-проверки;
> Classifier классифицирует страницы text/scan по отдельности и сохраняет
> deterministic-маппинг `type_code → reporting_standard` даже для отсканированных
> банковских РПБУ; Text Extractor собирает гибриды (нативный текст +
> постраничный OCR через Tesseract); Metric Extractor использует профили
> `bank` / `non_bank` из `config/metrics.yaml` и трёхуровневый приоритет
> источников `IFRS → RSBU → ISSUER`; Anthropic-прокол идёт с prompt-caching
> (1h TTL по умолчанию, включая `cache_hit_ratio` в логи). Перед первым
> `edx update` на новой инсталляции остаётся однократная задача оператора:
> заполнить `e_disclosure_id` в `config/tickers.yaml` (см. [§4.2](#42-сборка-configtickersyaml))
> и определиться с http-бэкендом Discoverer (см.
> [«ServicePipe / headless Chromium»](USER_GUIDE.md#servicepipe--headless-chromium)).
> Поэтапная логика парсинга, OCR и работы с LLM — в
> [`PIPELINE_LOGIC.md`](PIPELINE_LOGIC.md). Подробности по релизам — в разделе
...
| [`config/llm.yaml`](config/llm.yaml) | Claude Sonnet 4.6 как primary, OpenRouter как fallback, prompt-caching (`enable_prompt_caching` + `cache_ttl: 1h \| 5m`), дисковый кеш ответов |
...
### v3 — production-hardening после live-прогонов (Patch 23–28) — ✅ завершено

После пилотных запусков на боевом VPS вылезли инфраструктурные проблемы
(ServicePipe JA3, прокси для Anthropic/Drive из России, мелкие баги в
классификаторе и Metric Extractor под мультиэмитентным сценарием). Все
шесть патчей смерджены в master.

| # | Патч | Что приземлилось | Breaking? |
|---|---|---|---|
| 23 | Playwright-бэкенд для Discoverer/Downloader | headless Chromium один раз за прогон проходит JS-challenge ServicePipe; JA3 совпадает с настоящим Chrome; включается `discoverer.http_backend: playwright` | нет |
| 24 | HTTPS-прокси для Anthropic + Google Drive | `httplib2.proxy_info_from_environment` для Drive; e-disclosure обходит прокси через `NO_PROXY`; см. [USER_GUIDE §HTTPS-прокси](USER_GUIDE.md#https-прокси-для-anthropic--google-drive-patch-24) | нет |
| 25 | Classifier: deterministic `type_code` для scan-only PDF | банковский квартальный РПБУ (формы 0409806/0409807, чистый скан) больше не падает в `OTHER` — берётся `reporting_standard_for_type_code(pub.report_type_code)` | нет |
| 26 | Metric Extractor: dedup по `(date, period_type, std, metric_name)` | LLM иногда отдаёт две `extractions`-записи за один период; пайплайн схлопывает до одной строки (предпочитая non-null), не падая на `UNIQUE` constraint | нет |
| 27 | Drop comparative-period rows + UPSERT в `metrics_repo` | МСФО-отчёты IFRS Q1 несут comparative prior FY как отдельный блок; `extractions` с `period_type ≠ pub.reporting_period_type` отбрасываются; `INSERT OR REPLACE` чинит cross-publication коллизии | нет |
| 28 | Anthropic prompt-caching observability + 1h TTL | `cache_read_input_tokens` / `cache_creation_input_tokens` тянутся из `usage` и пишутся в `metric_extract_completed` вместе с `cache_hit_ratio`; новый `llm.primary.cache_ttl: 1h` (default) — кэш не испаряется в середине `edx run --full-reload` | нет |

**Внешние зависимости после v3:** сайт защищён ServicePipe (TLS-fingerprint +
JS-challenge). Стандартный путь — `discoverer.http_backend: playwright`
(headless-Chromium с автоматическим bootstrapping cookies). Установка
описана в [`USER_GUIDE.md → ServicePipe / headless Chromium`](USER_GUIDE.md#servicepipe--headless-chromium).
Альтернатива «httpx + ручное обновление cookies» оставлена в коде для
быстрых ручных прогонов, но в production не используется.

Перечень и принципы серии (мульти-эмитентные фикстуры, требование
источника к каждому синониму метрики и пр.) — в
[`prompts/README.md`](prompts/README.md). Поэтапная логика парсинга, OCR
и LLM, со всеми системными промптами и форматом ответов — в
