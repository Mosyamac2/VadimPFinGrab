...
│      ├── ocr (Tesseract rus+eng): pdf2image @ 300 DPI + pytesseract
...
1. PDF (или поднабор страниц для гибрида) рендерится в PNG c DPI =
   `config.ocr.tesseract_dpi` (default **300**).
2. Для каждой страницы вызывается `pytesseract.image_to_string(image,
   lang="rus+eng")` — языки берутся из `app.text_extractor.ocr_langs`.
3. Результат — список `PageText(page_number, text)`.
